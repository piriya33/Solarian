...
def build_astrology_prompt_context(
    ...
    lines = [
        ...
        "================================================================================",
        f"ชื่อเจ้าชะตา: {name}",
        f"วันเดือนปีเกิดตามสูติบัตร: {birth_date} เวลา {birth_time} น. สถานที่: {loc}",
        f"วันเกิดทางโหราศาสตร์ไทย: {day_res.get('day_name')} [{day_res.get('reason', '')}]",
        f"ลัคนาราศี (Ascendant): ราศี{chart_data['angles']['Ascendant']['sign_thai']} ({chart_data['angles']['Ascendant']['formatted_dms']})",
        f"สุริยราศี (Sun Sign): ราศี{chart_data['planets_dict']['Sun']['sign_thai']} ({chart_data['planets_dict']['Sun']['formatted_dms']})",
        f"จันทรราศี (Moon Sign): ราศี{chart_data['planets_dict']['Moon']['sign_thai']} ({chart_data['planets_dict']['Moon']['formatted_dms']})",
        "",
